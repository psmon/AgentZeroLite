...
3. **PDSA 4관점으로 합성** — **EN/KO 두 언어 동시 산출** (대시보드는 EN 기본 +
   KO 토글, 두 언어 다 비어 있으면 안 됨). 글로벌 audience 기준으로 EN 을 먼저
   쓰고, 같은 의미의 KO 를 한 줄씩 짝지어 작성:
   - **tried (Plan + Do)** : 무엇을 시도했나. 3~5 항목, 각 항목 `{ en, ko }`.
   - **solved** : 완료/확정된 것. 2~4 항목, 각 항목 `{ en, ko }`.
   - **remaining** : 미해결/차단/다음 사이클. 2~4 항목, 각 항목 `{ en, ko }`.
   - **learned (Study + Act)** : **가장 중요**. 반복된 Fail · 우연한 발견 · 재사용 가능한 프리미티브.
     - `lead` : 한 문장 핵심 통찰 (`{ en, ko }`)
     - `body` : 2~4문장 상세 근거 + 다음 액션 (`{ en, ko }`)

4. **`Home/harness-view/data/pdsa-insight.json` 덮어쓰기** (이중언어 스키마):
   ```json
   {
...
5. **git commit / push 는 사용자 명시 요청 시에만** — main 으로의 일반 push 는
   더 이상 Pages 를 건드리지 않는다 (harness/logs 잦은 변경으로 인한 무의미한
   재배포 방지). 운영 사이트에 반영하려면 **`doc-v*` 태그를 푸시**해야 한다.
...
```bash
# 1) 변경분이 main 에 모두 commit/push 된 상태에서
git tag doc-v1.0.0          # 새 doc 버전 (앱 버전 v* 와는 별개 namespace)
git push origin doc-v1.0.0  # 태그 push → pages.yml 발동 → 30~60초 후 Pages 갱신
```
...
- 사용자가 `harness/` · `Docs/` 에 파일을 추가/수정한 뒤 "뷰어에 반영" 류로 트리거하면,
...
- 매니페스트 (`Home/harness-view/indexes/*.json`) 는 아직 git tracked. CI 가
...
- **escape hatch**: GitHub Actions UI 에서 `pages.yml` 의 *Run workflow* 버튼
