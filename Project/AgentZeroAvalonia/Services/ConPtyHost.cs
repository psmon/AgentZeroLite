...
/// <summary>
/// The Windows PTY backend (M0035): a copy of the WPF host's <c>ManagedConPtyHost</c> —
/// a child process attached to a Windows pseudo-console over plain kernel32 P/Invoke
/// (<c>CreatePseudoConsole</c>, Windows 10 1809+), so the app ships no native terminal
/// DLLs. Kept as a copy rather than shared because the operator's rule for this branch
/// is "the WPF project is not touched", and this class is the proven piece.
///
/// Differences from the original are the seams only: it implements
/// <see cref="IPtyHost"/>, and the child's environment is handed in by the caller
/// (from <see cref="TerminalLaunchSpec.Env"/>) instead of built here.
/// </summary>
[SupportedOSPlatform("windows")]
internal sealed class ConPtyHost : IPtyHost
{
    ...
    /// <param name="environment">The child's environment — built, never inherited (see <see cref="TerminalEnvironment"/>).</param>
    ...
    public void Start(string commandLine, string? workingDir,
        IEnumerable<KeyValuePair<string, string>> environment, int cols, int rows)
    {
        lock (_sync)
        {
            if (_running) throw new InvalidOperationException("ConPTY host already started.");
            if (_disposed) throw new ObjectDisposedException(nameof(ConPtyHost));
            ...
            var size = new Native.COORD { X = Clamp(cols), Y = Clamp(rows) };
            ...
            // The child's environment is built by the caller, not inherited. Passing
            // IntPtr.Zero here would hand it whatever launched the GUI — which is how
            // NO_COLOR=1 from an IDE terminal once switched colour off in every tab.
            // CREATE_UNICODE_ENVIRONMENT is required because the block is UTF-16.
            var envBlock = TerminalEnvironment.ToBlock(environment);
            ...
            // A console child receives copies of the parent's standard handles even with
            // bInheritHandles=false (a long-standing compatibility rule). The WPF host
            // never noticed: a GUI process has none. A parent whose stdio is a pipe —
            // `-cli selftest`, a test runner — would hand that pipe to the child, and
            // cmd's output then bypasses the pseudo-console entirely (M0035 finding).
            // Blank our standard handles for the duration of CreateProcess so the child
            // has nothing to inherit and binds its stdio to the pseudo-console.
            ...
            // ConPTY keeps the output pipe open past child exit, so pipe EOF is not the
            // exit signal; the process handle is. One watcher per host, raising Exited
            // exactly once whichever signal lands first.
            var processHandle = _processHandle;
            _exitThread = new Thread(() =>
            {
                try { Native.WaitForSingleObject(processHandle, 0xFFFFFFFF); } catch { }
                _running = false;
                RaiseExited();
            }) { IsBackground = true, Name = "ConPtyExitWatch" };
            _exitThread.Start();
        }
    }
    ...
    private static short Clamp(int v) => (short)Math.Clamp(v, 1, short.MaxValue);
