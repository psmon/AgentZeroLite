...
1. **지연시간** — 계획(LLM) + 결정(Jev) 으로 턴당 왕복이 하나 늘어난다. 체감 가능한지.
2. **비용** — 요금 정보가 문서에 없음. `usage.input_tokens` 는 응답에 포함됨.
3. **`/v1/models` 경로** — SDK 에 `models.list()` 가 있으나 HTTP 경로는 문서에서 확인 못 함. 헬스체크 구현 전 확인 필요.
4. **`criteria` 개수 상한** — 2~4개 범위에선 문제없어 보이나 명시적 한도 미확인.
5. **한국어 state/criteria** 품질 — 문서 예시는 전부 영어. 한국어 요청에서 분류 품질 확인 필요.
6. **confidence 실제 분포** — 기본 경계값(0.6/0.85 등)은 **실측 후** 정해야 한다. 지금 정하면 추측이다.

## 9. 단계별 제안

| 단계 | 내용 | 가치 |
|---|---|---|
| **0** | 키 받는 즉시 `curl` 로 왕복 1회 + 지연시간 측정 | 위 미검증 항목 1·3·5 해소 |
| **1** | `IDecisionEngine` + `JevDecisionEngine` + `agent-one jev ask` 같은 CLI 검증 명령 | 루프 건드리지 않고 통합 검증 |
| **2** | TUI 4단계 + 헬스체크 + 키 저장 | operator 가 직접 설정 가능 |
| **3** | 계획 단계 + 결정 게이트를 `run`/`chat` 에 연결, Shift+Tab | 스마트 모드 동작 |
| **4** | confidence 경계값 실측 후 조정, 낮은 confidence 시 사용자 확인 UX | 신뢰성 |
