...
internal static class CliHandler
{
    ...
    private static int PrintHelp()
    {
        ...
        Console.WriteLine("  AgentZeroLite.ps1 terminal-alias set 0 1 build");
        Console.WriteLine("  AgentZeroLite.ps1 terminal-send --alias build \"dotnet build\"");
        Console.WriteLine("  AgentZeroLite.ps1 agent-resume-launch 0 1");
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  terminal-send <grp> <tab> <text>        Send text to a terminal (or --alias <name> <text>)");
        Console.WriteLine("  terminal-key  <grp> <tab> <key>         Send a control key to a terminal (or --alias <name> <key>)");
        Console.WriteLine("  terminal-read <grp> <tab> [--last N]    Read terminal output text (or --alias <name>)");
        Console.WriteLine("  terminal-wait <grp> <tab> [--until S]   Block until idle, or until state S (working|blocked|idle|done)");
        Console.WriteLine("  terminal-alias <list|set <g> <t> <name>|rm <name>>  Name a terminal so commands can target it via --alias");
        ...
        Console.WriteLine("  agent-resume <grp> <tab>                Print the resume command for a tab's workspace (auto-resolves cwd)");
        Console.WriteLine("  agent-resume-launch <grp> <tab>         Inject the resume command into the live terminal (or --alias <name>)");
