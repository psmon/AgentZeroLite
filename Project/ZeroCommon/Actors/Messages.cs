...
// ═══════════════════════════════════════════════════════════════
//
// AIMODE 추론은 AgentBotActor의 자식 AgentLoopActor에서 수행된다.
// (경로: /user/stage/bot/loop)
//
// 책임 분리:
//   - AgentBotActor : 채팅 UI 상태 + 첫 접촉 추적 + AgentLoop 자식 관리 (gateway)
//   - AgentLoopActor: IAgentLoop 라이프사이클 + Idle/Thinking/Acting/Done FSM
//                     + LLamaContext 보관 + 진행 상황 부모로 푸시 (the agent)
//
// UI ↔ 액터 통신은 모두 Tell 기반:
//   - SetAgentLoopCallbacks: UI delegate 등록 (Bot이 보관, AgentLoop 메시지 수신 시 호출)
//   - StartAgentLoop       : 사용자 발화로 새 turn loop 시작
//   - AgentLoopProgress    : Thinking/Acting/Generating 상태 변화 → UI 갱신
//   - AgentLoopResult      : 최종 응답(성공) 또는 실패 사유
//   - CancelAgentLoop      : 진행 중인 turn loop 취소
//   - ResetAgentLoopMemory : KV cache + introductions 리셋, 다음 Start에서 새 세션

/// <summary>UI에 AgentLoop 진행 상황을 전달할 콜백 등록 (UI → Bot).</summary>
public sealed record SetAgentLoopCallbacks(
    Action<AgentLoopProgress> OnProgress,
    Action<AgentLoopResult> OnResult);

/// <summary>AIMODE 추론 시작 (UI → Bot → AgentLoop).</summary>
public sealed record StartAgentLoop(string UserRequest);

/// <summary>AgentLoop 진행 상황 (AgentLoop → Bot → UI).</summary>
    ...
    int Round)
{
    ...
    public AgentLoopToolCallInfo? ToolCall { get; init; }

    /// <summary>Generating 단계 누적 토큰 수 (Phase=Generating일 때만).</summary>
    public int Tokens { get; init; }
}

/// <summary>AgentLoop 최종 결과 (AgentLoop → Bot → UI).</summary>
...
public sealed record AgentLoopToolCallInfo(
    string Tool,
    string ArgsJson,
    string Result);
...
/// <summary>진행 중 추론 취소 (UI → Bot → AgentLoop).</summary>
public sealed record CancelAgentLoop;

/// <summary>
/// 현재 AgentLoop 세션 폐기 (KV cache + agent loop dispose). 다음 StartAgentLoop가
/// 새 KV cache로 재시작하게 한다. UI의 "New Session" 버튼이 사용.
/// 부수 효과: AgentBotActor의 introductions도 함께 클리어.
/// </summary>
public sealed record ResetAgentLoopMemory;

/// <summary>
/// AgentLoop 자식 생성에 필요한 호스트 의존성. UI가 첫 SetAgentLoopCallbacks
/// 시점에 함께 전달.
///
/// <para><b>Backend-agnostic</b>: <see cref="AgentLoopFactory"/>는 활성 백엔드
/// (Local: LLamaSharp+GBNF / External: REST)를 보고 적절한
/// <see cref="Agent.Common.Llm.Tools.IAgentLoop"/> 구현을 만들어 돌려준다.
/// 액터는 어느 쪽인지 알 필요 없이 인터페이스만 호출.</para>
///
/// <para>팩토리는 (a) 액터가 OnTurnCompleted/OnGenerationProgress 콜백을
/// 주입한 옵션과 (b) 액터가 만든 toolbelt 인스턴스를 인자로 받아, 둘 다
/// 결합한 루프를 반환한다. 백엔드가 준비 안됐으면 (Local 미로드, External
/// 키 미설정 등) null 반환 — 액터는 실패 메시지를 부모에게 푸시하고 종료.</para>
/// </summary>
public sealed record AgentLoopBindings(
    Func<Agent.Common.Llm.Tools.IAgentToolbelt> ToolbeltFactory,
    Func<Agent.Common.Llm.Tools.AgentLoopOptions> OptionsFactory,
    Func<Agent.Common.Llm.Tools.AgentLoopOptions, Agent.Common.Llm.Tools.IAgentToolbelt,
         Agent.Common.Llm.Tools.IAgentLoop?> AgentLoopFactory);
...
// from MainWindow.HandleBotChat into the actor system so the agent loop
...
/// a continuation cycle on the agent loop. When inactive, peer signals
...
//   gets routed to the agent loop as a continuation trigger).
