...
## ⚠️ 보안 주의 — 반드시 먼저 읽어주세요

이 저장소를 **클론해서 빌드하거나 설치 파일을 실행하기 전에**, 코드를 직접 검증해 사용해주시길 강력히 권장드립니다.

- 본 앱은 **CLI를 직접 구동·중계**하는 구조이고, **AgentChatBot이 텍스트/키스트로크를 활성 터미널에 전달**합니다. 즉 **프롬프트 인젝션이 OS 명령 실행으로 이어질 수 있는 표면**이 존재합니다. 코드에 악성 동작 또는 인젝션을 악용하도록 유도하는 흔적이 없는지 **셀프 리뷰** 후 사용해주세요.
- AI에게 **OS 수준 제어**를 맡기는 도구를 만들면서 가드레일을 완전하게 거는 일은 저에게도 어려운 과제이며, 현재의 가드레일이 충분하다고 단언하지 않습니다.
- **설치 파일은 저(저장소 운영자)를 신뢰하는 분만 사용**해주세요. 이 프로젝트 구성을 본인이 안전하다고 판단하지 못한다면 **설치를 권장하지 않습니다.**
- 저는 이 프로젝트를 **제품으로 홍보하지 않으며, 기술 연구 목적**으로만 진행합니다. 보안 이슈/개선 제안은 이슈로 알려주시면 감사하겠습니다.

---

## 🧪 프로젝트 컨셉 — 무엇을 실험 중인가

AgentZero Lite는 **온디바이스 AI 시대를 대비**해 .NET을 활용해 — 당장은 **Windows 전용으로** 시도되고 있는 — **실험 저장소**입니다.

- 최근 npm 오픈소스 생태계에서 연이은 보안 사태가 터지면서, **그래도 MS가 관리하는 NuGet / winget 같은 채널**은 AI CLI/OS 네이티브 시대에 **보안·안정성을 무기로 한 네이티브 영역의 틈새**가 있다고 판단했습니다. 닷넷의 수많은 스펙 중 *네이티브에 가까운 영역*에서, npm이 장악한 시장에 **MS 관리 채널 기반의 안정 네이티브 기술**로 비집고 들어갈 자리가 있다고 보고, 다시 닷넷을 잡기 시작했습니다.
- 리눅스 진영은 이미 쓸 만한 AI Agent CLI들이 **거의 macOS 우선**으로 계속 출시되고 있어, **Windows 네이티브가 AI 시대에 끼어들 틈이 있는가?** 를 함께 실험하는 저장소입니다. Windows 네이티브용 좋은 Agent CLI가 부족한 현실에서, 닷넷 기반으로 그 공백을 직접 채워보려는 시도입니다.
- 따라서 본 저장소는 **상용 제품이 아니라 살아 있는 연구 노트**에 가깝습니다. 빠르게 실험하고, 막힌 곳은 문서로 남기며 (`Docs/llm/` 등), 공식 NuGet 지원이 따라잡으면 우회 코드를 즉시 걷어내는 방식으로 운영됩니다.
